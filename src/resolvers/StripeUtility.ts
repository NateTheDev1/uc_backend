...
export const charge = async (args: Resolvers.MutationCreateOrderArgs) => {
	...
	for (let i = 0; i < order.cart.length; i++) {
		formattedDescription += `${order.cart[i]?.name}, quantity: ${order.cart[i]?.quantity.toString()}` + " ";
	}
	...
	try {
		...
		const orderConfirmation = await Orders.query().insertAndFetch({ user_id: order.user.id, orderTitle: order.description });
