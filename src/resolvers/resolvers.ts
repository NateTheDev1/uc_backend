...
const resolvers: Resolvers.Resolvers = {
  Query: {
    product: async (
      parents,
      args: Resolvers.QueryProductArgs,
    ...
    products: async (
      parent,
      args: Resolvers.QueryProductsArgs,
      context: Server.Context
    ) => {
      const { productGroupId } = args;

      context.logger.info(
        "Query: products for product group with id of " + productGroupId
      );
  ...
  Mutation: {
    createUser: async (parent, args, context: Server.Context) => {
      ...
      context.logger.info(
        "Trying to create user with username: " + user.username
      );
      ...
      const created = await User.query().insertAndFetch({
        ...user,
        password: await authenticationUtility.hashPassword(user.password),
      });
    ...
    loginUser: async (parent, args, context: Server.Context) => {
      ...
      context.logger.info(
        "Trying to login user with username of " + credentials.username
      );

      const foundUser = await User.query().where({
      ...
      const valid = await authenticationUtility.verifyPassword(
      ...
      if (valid && context.res) {
        const JWTSession = await authenticationUtility.createSession(
          foundUser[0]
        );
    ...
    createProduct: async (
      parent,
      args: Resolvers.MutationCreateProductArgs,
      context: Server.Context
    ) => {
      context.logger.info(
        "Mutation: creating product with name of " + args.product.name
      );
    ...
    createProductGroup: async (
      parent,
      args: Resolvers.MutationCreateProductGroupArgs,
      context: Server.Context
    ) => {
    ...
    deleteProduct: async (
      parent,
      args: Resolvers.MutationDeleteProductArgs,
      context: Server.Context
    ) => {
    ...
    editConfig: async (parent, args, context: Server.Context) => {
      ...
      config = await Config.query().upsertGraphAndFetch({
        id: id.toString(),
        value,
      });
    ...
    removeAdmin: async (
      parent,
      args: Resolvers.MutationRemoveAdminArgs,
      context: Server.Context
    ) => {
      try {
    ...
    editProduct: async (
      parent,
      args: Resolvers.MutationEditProductArgs,
      context: Server.Context
    ) => {
    ...
    createOrder: async (
      parent,
      args: Resolvers.MutationCreateOrderArgs,
      context: Server.Context
    ) => {
